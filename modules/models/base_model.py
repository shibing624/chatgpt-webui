...
class BaseLLMModel:
    ...
    def summarize_index(self, files, chatbot, language):
        ...
        if files:
            ...
            summary = \
            chain({"input_documents": list(index.docstore.__dict__["_dict"].values())}, return_only_outputs=True)[
                "output_text"]
    ...
    def prepare_inputs(self, real_inputs, use_websearch, files, reply_language, chatbot):
        ...
        if files:
            ...
            reference_results = [[d.page_content.strip("�"), os.path.basename(d.metadata["source"])] for d in
                                 relevant_documents]
    ...
    def predict(
        ...
        limited_context, fake_inputs, display_append, inputs, chatbot = self.prepare_inputs(real_inputs=inputs,
    ...
    def reset(self):
        ...
        self.all_token_counts = []
        self.interrupted = False
        pathlib.Path(os.path.join(HISTORY_DIR, self.user_identifier,
                                  new_auto_history_filename(os.path.join(HISTORY_DIR, self.user_identifier)))).touch()
